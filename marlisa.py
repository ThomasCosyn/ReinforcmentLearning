...
class MARLISA:
    ...
    def __init__(self, building_ids, 
        ...
        for uid, info in building_info.items():
            _coef = info['annual_dhw_demand']/.9 + info['annual_cooling_demand']/3.5 + info['annual_nonshiftable_electrical_demand'] - info['solar_power']*8760/6.0
            self.energy_size_coef[uid] = max(.3*(_coef + info['solar_power']*8760/6.0), _coef)/8760
        ...
        for uid in building_ids:
            ...
            if building_info[uid]['solar_power'] == 0:
                for k in range(12,20):
                    if self.encoder[uid][k] != 0:
                        self.encoder[uid][k] = -1
                if self.encoder[uid][24] != 0:
                    self.encoder[uid][24] = -1
            if building_info[uid]['annual_dhw_demand'] == 0 and self.encoder[uid][26] != 0:
                self.encoder[uid][26] = -1
            if building_info[uid]['annual_cooling_demand'] == 0 and self.encoder[uid][25] != 0:
                self.encoder[uid][25] = -1
            if building_info[uid]['annual_nonshiftable_electrical_demand'] == 0 and self.encoder[uid][23] != 0:
            ...
            if building_info[uid]['annual_dhw_demand'] == 0 and self.encoder_reg[uid][26] != 0:
                self.encoder_reg[uid][26] = -1
            if building_info[uid]['annual_cooling_demand'] == 0 and self.encoder_reg[uid][25] != 0:
                self.encoder_reg[uid][25] = -1
            if building_info[uid]['annual_nonshiftable_electrical_demand'] == 0 and self.encoder_reg[uid][23] != 0:
